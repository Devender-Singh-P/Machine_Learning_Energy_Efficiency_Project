...
class DataIngestion:
    ...
    def download_energy_efficiency_data(self,) -> str:
        try:
            ...
            tgz_file_path = os.path.join(
                tgz_download_dir, energy_efficiency_file_name)
            logging.info(
    ...
    def extract_tgz_file(self, tgz_file_path: str):
        try:
            ...
            #    energy_efficiency_tgz_file_obj.extractall(path=raw_data_dir)
    ...
    def split_data_as_train_test(self) -> DataIngestionArtifact:
        try:
            ...
            energy_efficiency_data_frame = pd.read_csv(
                energy_efficiency_file_path)
            ...
            for train_index, test_index in split.split(energy_efficiency_data_frame, energy_efficiency_data_frame["load_cat"]):
                strat_train_set = energy_efficiency_data_frame.loc[train_index].drop([
                                                                                     "load_cat"], axis=1)
                strat_test_set = energy_efficiency_data_frame.loc[test_index].drop([
                                                                                   "load_cat"], axis=1)
